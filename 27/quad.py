...
def generatePrimes(limit=1000000):
    sieve = [False] * limit
    primes = []
...
# the following is using the method described at: https://www.mathblog.dk/project-euler-27-quadratic-formula-primes-consecutive-values/
    ...
    for a in range(-999, 1000, 2):
        for i in range(1, len(bPos)):
            for j in range(0, 2):
                ...
                aodd = -1 if i % 2 == 0 else 0
                #while is_prime(abs((n**2) + a*n + b)):
                while is_prime(abs((n**2) + (a+aodd) * n + sign*bPos[i])):
                    n+=1
                if n > maxN:
                    print("New max of %d" % (n))
                    maxA=a
                    maxB=bPos[i]
                    maxN=n
    print("A sequence of length %d, is generated by a=%d, b=%d the product is %d" % (n,a,b,a*b))
    print("Solution took %d seconds" % (time()-start))
            ...
            for n in range(10000000):
                ...
                # if the next number is not prime, then we are done finding consecutive primes
                    ...
                    if len(primes) > maxlen:
                        ...
                        maxA = a
                        maxB = b
                        print("CURRENT MAXLEN:\n\t" + str(maxlen))
                        sleep(1)
                    # skip to next equation
                    break
